 ...
 * \file cf_db_mysql.h
 * \brief mysql 관련 공통라이브러리 파일. 기존 코드를 수정했음.
 */
...
/** 쿼리를 실행하고 첫번째 선택된 값을 문자열형으로 받아옴. 문자열의 길이는 충분하다고 가정. */
...
#define cf_db_exec(d,q,f,n,e)  (ret_t)mysql_db_exec(&(d)->db,q,e)
#define cf_db_busy_handler(d,f,m) 
#define cf_db_busy_timeout(d,tm) 

// mysql 인터페이스 함수
extern MYSQL_BIND	*g_pBind ;
#define cf_db_prepare_v2(d,q,l,st,t) (ret_t)mysql_prepare(d,q,l,st,t)
#define cf_db_step(st) (ret_t)mysql_step(st)
...
#define cf_db_get_table(d,q,v,r,c,e) (ret_t)mysql_get_table(&(d)->db, q, v, r, c, e)
